...
export class ImprovisedSpellDialogue extends FormApplication {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      template: "systems/vampire-the-requiem-2e/templates/dialogues/dialogue-improvisedSpell.html",
      resizable: true
    });
  }
  ...
  getData() {
    ...
    const isAnyMage = actorData.characterVariant === "mage" || actorData.characterVariant === "scelesti" || this.actor.type === "ephemeral";
    const isEphemeral = this.actor.type === "ephemeral";
    if (actorData.characterVariant === "scelesti") data.scelestiRank = actorData.scelestiRank;
  ...
  _saveAsTemplate() {
    ...
    if (!spellData.img || spellData.img === "icons/svg/item-bag.svg" || spellData.img.startsWith('systems/vampire-the-requiem-2e/icons/placeholders')) {
    ...
    ui.notifications.info("Template created for " + this.actor.name);
  }
  ...
  async getChatData(spell) {
    ...
    data.description = await foundry.applications.ux.TextEditor.implementation.enrichHTML(data.description, { secrets: secrets, entities: true });

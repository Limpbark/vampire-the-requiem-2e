    ...
    const actorTilts = (this.actor?.items.filter(item => {
      if (item.type === "tilt" || item.type === "condition") {
        if (item.system.statusVisibility === "All" || (item.system.statusVisibility === "Owner" && this.actor.isOwner)) {
          if (item.type === "condition" && !game.settings.get("vampire-the-requiem-2e", "showConditionsOnTokens")) return false;
          return true;
        }
      }
    })) || [];
  ...
  _drawBar(number, bar, data) {
    if (data.attribute === 'health') {
      ...
      if (health && health.lethal !== undefined && health.aggravated !== undefined) {
        ...
        const blk = 0x000000;
        let color_health = parseInt(game.settings.get("vampire-the-requiem-2e", "tokenHealthColorHealthy").replace('#', '0x'), 16);
        let color_aggr = parseInt(game.settings.get("vampire-the-requiem-2e", "tokenHealthColorAggravated").replace('#', '0x'), 16);
        let color_lethal = parseInt(game.settings.get("vampire-the-requiem-2e", "tokenHealthColorLethal").replace('#', '0x'), 16);
        let color_bashing = parseInt(game.settings.get("vampire-the-requiem-2e", "tokenHealthColorBashing").replace('#', '0x'), 16);
        // Draw the bar
        bar.clear()

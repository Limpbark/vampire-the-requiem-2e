export const registerCustomSelectBoxes = async function (html, parentSheet) {
  ...
  parentEle.querySelectorAll('input[data-koptions]').forEach(ele => {
    if (!ele.classList.contains('fancyComboBox')) {
      ele.classList.add('fancyComboBox');
      ele.insertAdjacentHTML('afterend', `<div>${ele.dataset.koptions.split('/').reduce((a, v) =>
        a + `<div${v.match(/^-+$/) ? ' class="kOptionDelimiter"' : v.match(/^--.+--$/) ? ' class="kOptionHeader"' : ''}>${v.match(/^-+$/) ? '' : (v.match(/^--.+--$/)) ? v.slice(2, -2) : v
        }</div>`, '')}</div>`);
      ...
      let fillEle = (sourceEle, blurBool = false) => {
        ele.value = sourceEle.innerText; if (blurBool) { ele.blur() }
      ...
      ele.addEventListener('keydown', e => {
        if (['ArrowDown', 'ArrowUp', 'Enter'].includes(e.key)) {
          if (e.key == "Enter" && activeEle && activeEle.parentNode) {
            ...
            let cLength = dropDown.children.length;
            let attempts = 0;
            do {
              eIndex = eIndex + ((e.key == 'ArrowDown') ? 1 : -1);
              eIndex = (eIndex >= cLength) ? 0 : (eIndex < 0) ? cLength - 1 : eIndex;
              attempts++;
            } while (attempts < cLength && ['kOptionDelimiter', 'kOptionHeader'].some(v => dropDown.children[eIndex].classList.contains(v)));

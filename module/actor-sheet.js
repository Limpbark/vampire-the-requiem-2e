...
const getInventory = () => ({
  ...
  echo: {
    dataset: [{label: "MTA.EchoWyrdRating", property: "system.rating"}]
  },
  form: {
    descriptions: {},
    dataset: []
  ...
  affinity: {
    dataset: [ {label: "MTA.Mummy.Affinity.pillar"} ]
  },
  utterance: {
    dataset: [ {label: "MTA.Mummy.Utterance.pillars"} ]
  },
  scar: {
    dataset: [{ label: "MTA.Magnitude", property: "system.magnitude" }, { label: "MTA.Overt", property: "system.overt" }, { label: "MTA.Type", property: "system.type" }, { label: "MTA.ActivationMethod.Title", property: "system.activationMethod" }]
  },
  variation: {
    dataset: [{ label: "MTA.Magnitude", property: "system.magnitude" }, { label: "MTA.Overt", property: "system.overt" }, { label: "MTA.ActivationMethod.Title", property: "system.activationMethod" }, { label: "MTA.AttachedScar", property: "system.scar" }]
  },
  adaptation: {
    dataset: [{ label: "MTA.Used", property: "system.used" }]
  },
  conviction: {
    dataset: [{ label: "MTA.Wavering", property: "system.wavering" }]
  },
  loyalty: {
    dataset: [{ label: "MTA.Wavering", property: "system.wavering" }]
  },
  alembic: {
    dataset: [{ label: "MTA.Transmutation", property: "system.transmutation"}, { label: "MTA.Distillations", property: "distillations"}, { label: "MTA.Calcified", property: "isCalcified"}]
  },
  distillation: {
    dataset: [{ label: "MTA.Action", property: "system.action"}, { label: "MTA.Cost", property: "system.cost"}, { label: "MTA.Alembic", property: "system.alembic"}]
  },
  bestowement: {
    dataset: []
  },
  role: {
    dataset: [{ label: "MTA.Refinement", property: "system.refinement"}, { label: "MTA.OnPilgrimage", property: "system.isOnPilgrimage"}, { label: "MTA.IsCompleted", property: "system.isCompleted"}]
  },
  milestone: {
    dataset: [{ label: "MTA.Type", property: "system.type"}, { label: "MTA.Magnitude", property: "system.magnitude"}, { label: "MTA.IsCompleted", property: "system.isCompleted"}]
  },
});
...
export class MtAActorSheet extends foundry.appv1.sheets.ActorSheet {
   ...
   */
  get template() {
    if (!game.user.isGM && this.actor.limited) return "systems/vampire-the-requiem-2e/templates/actors/limited-sheet.html";
    if (this.actor.type === "ephemeral") return "systems/vampire-the-requiem-2e/templates/actors/ephemeral-sheet.html";
    if (this.actor.type === "simple_antagonist") return "systems/vampire-the-requiem-2e/templates/actors/simple-antagonist-sheet.html";
    if (this.actor.type === "brief_nightmare") return "systems/vampire-the-requiem-2e/templates/actors/brief-nightmare-sheet.html";
    return "systems/vampire-the-requiem-2e/templates/actors/character.html";
  }
  ...
  async getData() {
    ...
    const characterConf = CONFIG.MTA.characterConfig[this.actor.type];
    if(characterConf) {
      // Build localisation for character types (takes the locale string of the first variant)
      for (const [typeKey, variants] of Object.entries(characterConf)) {
        const firstVariant = variants && Object.values(variants)[0];
        sheetData.characterTypes[typeKey] =
          (firstVariant && firstVariant.locale) || typeKey;
      }
    }
    ...
    if (sheetData.show.clarity) inventory.condition.dataset.push({ label: "MTA.Clarity" });
    ...
    //Inventory sorting
    const sortFlags = game.user.flags?.["vampire-the-requiem-2e"]?.sort;
    if (sortFlags) {
    ...
    if (sheetData.show.manaMage) sheetData.mana_per_turn = CONFIG.MTA.gnosis_levels[Math.min(9, Math.max(0, systemData.mage_traits.gnosis.final - 1))].mana_per_turn;
    else if (sheetData.show.manaProximi) sheetData.mana_per_turn = 1;
    if (systemData.characterVariant === "vampire" || systemData.characterVariant === "ghoul") sheetData.vitae_per_turn = CONFIG.MTA.bloodPotency_levels[Math.min(10, Math.max(0, systemData.vampire_traits.bloodPotency.final))].vitae_per_turn;
    
    if (sheetData.show.glamour) {
      sheetData.glamour_per_turn = CONFIG.MTA.glamour_levels[Math.min(9, Math.max(0, systemData.changeling_traits.wyrd.value - 1))].glamour_per_turn;
    }

    if (sheetData.show.glamourHobgoblin) {
      sheetData.glamour_per_turn = CONFIG.MTA.glamour_levels_hobgoblin[Math.min(9, Math.max(0, systemData.changeling_traits.wyrd.value - 1))].glamour_per_turn;
    }
    ...
    if(systemData.pandoranType && systemData.pandoranType === game.i18n.localize(CONFIG.MTA.pandoranTypes.praecipitatus))
      sheetData.isPraecipitatus = true;
    else sheetData.isPraecipitatus = false;
    ...
    if (sheetData.show.spells || sheetData.show.blessings || sheetData.show.supernalSpells) {
      sheetData.activeSpells = {
        ...
        max: systemData.mage_traits.activeSpellMaximum.final
      };
    }

    if (sheetData.show.blessings) {
    ...
    let derivedTraits = {...CONFIG.MTA.derivedTraits};
    if (systemData.characterType === "deviant") {
      derivedTraits.stability = game.i18n.localize("MTA.deviant_traits.stability");
    }
    ...
    this.actor.items.forEach(item => {
      ...
      if (item.type === "affinity") {
        item.pillar_loc = game.i18n.localize("MTA.Mummy.pillar." + item.system.pillar);
      }
      if (item.type === "utterance") {
        ...
        for (let i = 1; i <= 3; i++) {
          ...
          const pillarName = game.i18n.localize(
            `MTA.Mummy.pillar.${system[`${tier}_pillar`]}`
          );

          const level = system[`${tier}_pillar_level`];
          const keywords = system[`${tier}_pillar_keywords`] ?? [];

          const keywordText = keywords.length
            ? ` (${keywords
              .map(kw => {
                const text = game.i18n.localize(`MTA.Mummy.Utterance.${kw}`);
                return kw === "potency"
                  ? `${text} ${system[`${tier}_potency`]}`
                  : text;
              })
              .join(", ")})`
            : "";

          pillars.push(`${pillarName} ${level}${keywordText}`);
        }

        item.pillarsString = pillars.join(", ");
      }

      if (['scar', 'variation'].includes(item.type)) {
        item.activationMethod = game.i18n.localize(CONFIG.MTA.activationMethods[item.system.activationMethod]);
      }
      if (item.type == "scar") {
        item.scarAttribute = game.i18n.localize(CONFIG.MTA.scarTypes[item.system.type]);
      }
      if (item.type == "variation") {
        item.attachedScars = item.system.scar;
      }
      if (item.type === "distillation") {
        item.actionName = game.i18n.localize(CONFIG.MTA.actionTypesSelection[item.system.action]);
        const alembicParents = this.actor.itemTypes.alembic.filter(d => item.system.alembic === d.name);
        if(alembicParents.length && alembicParents[0].system.isCalcified) {
          item.isCalcified = true;
        }
        else {
          item.isCalcified = false;
        }
      }
      if(item.type === "alembic") {
        const distillations = this.actor.itemTypes.distillation;
        item.distillations = distillations.filter(d => d.system.alembic === item.name).length;
      }

      if(item.type === "milestone") {
        if(item.system.isHidden && !game.user.isGM)
          item.show = false;
        else
          item.show = true;

        item.lType = game.i18n.localize(CONFIG.MTA.prometheanMilestoneTypes[item.system.type]);
        item.lMagnitude = game.i18n.localize(CONFIG.MTA.prometheanMilestoneMagnitudes[item.system.magnitude]);
      }
    });
    ...
    /* Get ephemeral entity nickname */
    if (actor.type === "ephemeral") {
      if (systemData.characterType === "Supernal Entity") {
        // TODO: Not implemented..
        sheetData.ephemeralEntityName = game.i18n.localize(`MTA.ephemeralRanks.supernalEntity.${systemData.supernalArcanum1}`);
        ...
        if (systemData.sineaterId) sheetData.ephemeralEntityName = game.i18n.localize("MTA.ephemeralGeist");
        else if (systemData.eph_general.rank.final > 10) sheetData.ephemeralEntityName = game.i18n.localize("MTA.ephemeralHigher");
        else if (systemData.eph_general.rank.final < 1) sheetData.ephemeralEntityName = game.i18n.localize("MTA.ephemeralLesser");
        else {
          const localeKey = `MTA.ephemeralRanks.${systemData.characterType}.${systemData.eph_general.rank.final}`;
          sheetData.ephemeralEntityName = game.i18n.localize(localeKey);
          
          // No translation exists -> Use character type
          if(sheetData.ephemeralEntityName === localeKey) {
            sheetData.ephemeralEntityName = game.i18n.localize(CONFIG[systemData.typeConf.characterVariants[systemData.characterVariant]]);
          }
        }
      }
    }
    ...
    if (actor.type === "character") {
      ...
      for (const item of inventory.general_dice_pool.items) {
        item.system.enrichedDescription = await foundry.applications.ux.TextEditor.implementation.enrichHTML(item.system.description, { secrets: this.actor.isOwner, entities: true });
      }
    ...
    sheetData.enrichedDescription = await foundry.applications.ux.TextEditor.implementation.enrichHTML(this.actor.system.description, { secrets: this.actor.isOwner, entities: true });
    ...
    console.log(data)
    data.owodAttributeOrdering = game.settings.get("vampire-the-requiem-2e", "owodAttributeOrdering");
  ...
  activateListeners(html) {
    ...
    html.find('.item-table .cell.header.first .collapsible.button').toArray().filter(ele => {
      if (this.actor && this.actor.id && game.user.flags["vampire-the-requiem-2e"] && game.user.flags["vampire-the-requiem-2e"][this.actor.id] && game.user.flags["vampire-the-requiem-2e"][this.actor.id][$(ele).siblings('.sortable.button')[0].dataset.type] && game.user.flags["vampire-the-requiem-2e"][this.actor.id][$(ele).siblings('.sortable.button')[0].dataset.type].collapsed) {
    ...
    html.find('.item-table .super-sort.button').toArray().forEach(ele => {
      ...
      if (!itemType || !game.user.flags["vampire-the-requiem-2e"]?.sort) return;

      const sortData = game.user.flags["vampire-the-requiem-2e"].sort[itemType];
    ...
    html.find('.item-table .sortable.button').toArray().forEach(ele => {
      ...
      const itemType = ele.dataset.sorttype;
      if (!property || !itemType || !game.user.flags["vampire-the-requiem-2e"]?.sort) return;
...
/* 
    ...
    html.find('.hiddenIcon').click(ev => {
      const itemId = ev.currentTarget.closest(".hiddenIcon").dataset.itemId;
      const item = this.actor.items.get(itemId);
      let toggle = !item.system.isHidden
      const updateData = {
        "system.isHidden": toggle
      };
      item.update(updateData);
    });

    html.find('.item-property-checkbox input').click(ev => {
      const dataset = ev.currentTarget.dataset
      const itemId = dataset?.itemId;
      const property = dataset?.property;

      if(!itemId || !property)
        return console.error("Missing itemId or property", itemId, property);

      const item = this.actor.items.get(itemId);
      if(!item)
        return console.error("Item not found", itemId);

      const state = foundry.utils.getProperty(item, property);
      if(state == undefined)
        return console.error("Property not found", item, property);
    ...
    html.find('.devoted-warning').click(ev => {
      const updateData = {
        "system.isDevoted": !this.actor.system.isDevoted
      };
...
/*     html.find('.transmutation-create').click(ev => {
      let ownTransmutations = this.actor.system.transmutations_own ? foundry.utils.duplicate(this.actor.system.transmutations_own) : {};
      var randomId = foundry.utils.randomID();
      let transmutation = {
        label: "New Transmutation",
        value: 0,
        alembics: {
          [randomId]: {
      ...
      let newKey = Object.keys(ownTransmutations).reduce((acc, ele) => {
        return +ele > acc ? +ele : acc;
      }, 0);
      newKey += 1;
      ownTransmutations[newKey] = transmutation;
      const updateData = {
        "system.transmutations_own": ownTransmutations
      };
      this.actor.update(updateData);
    });

    html.find('.specialization-delete').click(ev => {
      var type = ev.currentTarget.dataset.type ?? null;
      if (!type) return;
      let ownSpecializations = this.actor.system[type] ? foundry.utils.duplicate(this.actor.system[type]) : {};
      const specialization_key = ev.currentTarget.dataset.attribute;

      ownSpecializations[`-=${specialization_key}`] = null;
      let updateData = {
        [`system.${type}`]: ownSpecializations
      };
      this.actor.update(updateData);
    });

    html.find('.specialization-name.invisible').toggle();

    html.find('.specialization-edit').click(ev => {
      const et = $(ev.currentTarget);
      et.siblings(".specialization-name").toggle();
      et.siblings(".attribute-button").toggle();
    });

    $('.specialization-edit').on('keypress', function (e) {
      if (e.which === 13) {
        const et = $(ev.currentTarget);
        et.siblings(".specialization-name").toggle();
        et.siblings(".attribute-button").toggle();
    ...
    html.find('.pillar-state').click(ev => {
      const traitName = ev.currentTarget.closest(".pillar-state").dataset.trait;
      ...
      if (updateAttribute[haunt].isPrimary) {
        updateAttribute[haunt].isPrimary = false;

      } else {
        updateAttribute[haunt].isPrimary = true;
      }
    ...
    html.find('.falteringButton').mousedown(ev => {
      this.actor.falteringRoll();
    });
  ...
  _onDropActor(event, data) {
    if (this.actor.system.characterType === "sinEater" && data.uuid) {
    ...
    else if (this.actor.system.characterType === "ghost" && data.uuid) {
      const actor = fromUuidSync(data.uuid);
      if (actor && actor.id) {
        this.actor.update({
  ...
  */
  ...
  async _onDropSingleItem(itemData) {
    ...
    if (itemData.system) {
      ...
      if (itemData.system.isCalcified) itemData.system.isCalcified = false;
      if(itemData.type === "role") {
        itemData.system.isCompleted = false;
        itemData.system.isOnPilgrimage = false;
      }
      if(itemData.type === "milestone") {
        itemData.system.isHidden = false;
        itemData.system.isCompleted = false;
      }
    }
  ...
  async _onItemCreate(event) {
   ...
   // if (event.currentTarget.dataset?.subtype) itemData.system = { subtype: event.currentTarget.dataset.subtype };
  ...
  _onItemRoll(event, quickRoll = false) {
    ...
    if (item.type === "utterance") return this.actor.utterUtterance(item);
  ...
  _onTableCollapse(event) {
    ...
    if (et.hasClass('fa-plus-square')) { // collapsed
      ...
      let updateData = { 'flags': { 'vampire-the-requiem-2e': { [this.actor._id]: { [et.siblings('.sortable.button')[0].dataset.type]: { collapsed: false } } } } };
      ...
      // Update user flags, so that collapsed state is saved
      let updateData = { 'flags': { 'vampire-the-requiem-2e': { [this.actor._id]: { [et.siblings('.sortable.button')[0].dataset.type]: { collapsed: true } } } } };
      game.user.update(updateData);
    }
  }
  ...
  async _onSuperSort(event) {
    ...
    const updateData = {
      'flags.vampire-the-requiem-2e.sort': {
  ...
  async _initialiseDotTrackers(html) {
    ...
    trackers.toArray().forEach(trackerEle => {
      if (trackerEle.dataset && !trackerEle.dataset.initialised) {
        ...
        let trackerAddMax = (td.addmax) ? false : true;
        ...
        let inputs = stateOrder.map((v, k) => {
          if (k === 0 && this.options.editable && trackerAddMax) {
        ...
        let updateDots = (num = 0, index = false) => {
          ...
          if (trackerType == 'health' && !(this.actor.type === "ephemeral")) {
            ...
            let penaltyString = penaltyMap[dots[dots.length - 1]];
            if(this.actor.system.typeConf.sheet.noLosingCounsciousnessOrBleeding && dots[dots.length - 1] > 0 && dots[dots.length - 1] < 3)
              penaltyString = penaltyMap[0];
            r += `<div class="info"><span>${game.i18n.localize('MTA.HealthPenalty.YouAre')} <b>${penaltyString}</b>.</span><span>${game.i18n.localize('MTA.DicePenalty')}<b>−${dicePenalty}</b></span></div>`;
          }
          ...
          else if (trackerType == 'stability') {
            const penalties = this.actor.getStabilityPenalties();
            r += `<div class="info"><span>${game.i18n.localize('MTA.DicePenalty')}<b>−${penalties.stabilityPenalty}</b></span></div>`;
            if(penalties.endStage)
              r += `<div class="info"><span><b>${game.i18n.localize('MTA.EndStage')}</b></span></div>`;
            else {
              if(penalties.scarMagnitude)
                r += `<div class="info"><span><b>${game.i18n.localize('MTA.ScarMagnitude')}<b>−${penalties.scarMagnitude}</b></span></div>`;

              if(penalties.newScars)
                r += `<div class="info"><span><b>${game.i18n.localize('MTA.NewScar')}<b>−${penalties.newScars}</b></span></div>`;
            }
          }
        ...
        if (this.options.editable) {
          ...
          trackerEle.addEventListener('change', (e, t = e.target) => {
  ...
  async _updateObject(event, formData) {
    ...
    // Character type changed -> Reset character variant
    if(formData.system?.characterType && formData.system.characterType !== this.actor.system.characterType) {
      const characterConf = CONFIG.MTA.characterConfig[this.actor.type]?.[formData.system.characterType];
      if(characterConf) {
        const variants = Object.keys(characterConf);
        if(variants) {
          formData.system.characterVariant = variants[0];
        }
      }
    }

    if (formData.system.touchstones_changeling) {
